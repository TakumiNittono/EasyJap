...
import { katakanaCharacters, katakanaList } from '@/lib/stroke-data/katakana';
...
export default function TracePracticePage({
  ...
  // 前後の文字を取得
  const currentIndex = katakanaList.indexOf(character);
  const prevCharacter = currentIndex > 0 ? katakanaList[currentIndex - 1] : null;
  const nextCharacter = currentIndex >= 0 && currentIndex < katakanaList.length - 1 ? katakanaList[currentIndex + 1] : null;
  ...
  useEffect(() => {
    // 文字データを取得
    const found = katakanaCharacters.find(c => c.character === character);
    if (found) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-3 sm:p-4 md:p-6 mb-4 sm:mb-6">
          ...
          <div className="flex items-center justify-between gap-3 sm:gap-4 mt-4 sm:mt-6 px-2 sm:px-4">
            {prevCharacter ? (
              <Link
                href={`/katakana/${encodeURIComponent(prevCharacter)}/trace`}
            ...
            {nextCharacter ? (
              <Link
                href={`/katakana/${encodeURIComponent(nextCharacter)}/trace`}
